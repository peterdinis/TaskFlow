...
export function TodoProvider({ children }: { children: ReactNode }) {
    ...
    // Queries
    const rawTasks = useQuery(api.todos.getTodosByUser, user ? { userId: user.id as Id<"users"> } : "skip");
    const rawProjects = useQuery(api.projects.getProjectsByUser, user ? { userId: user.id as Id<"users"> } : "skip");
    const rawLabels = useQuery(api.labels.getLabelsByUser, user ? { userId: user.id as Id<"users"> } : "skip");
    ...
    useEffect(() => {
        if (rawTasks && rawProjects && rawLabels) {
            ...
            setIsLoading(false);
        } else if (rawTasks === null || rawProjects === null || rawLabels === null) {
            setIsLoading(false);
        }
